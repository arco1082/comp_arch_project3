...
class CacheTracker {
private:
    std::set<uint32_t> trackerList;
public:

    bool isCompulsoryMiss(uint32_t tag) {
        if(trackerList.find(tag) == trackerList.end()) { 
            trackerList.insert(tag);
            return true;
        }
        return false;
    }
