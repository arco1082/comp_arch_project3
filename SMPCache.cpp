...
void SMPCache::doRead(MemRequest *mreq)
{
    ...
    GI(l, !l->isLocked());
    //PROJECT 3 - Code
    if (l && ((l->getState() & DMESI_TRANS) || l->hasInvalidBit())) {
    	readCoheMiss.inc();
    } else {
        std::vector<PAddr>::iterator it;	  	
    	PAddr currentTag = calcTag(addr);
    	it = std::find(invalidTags.begin(), invalidTags.end(), currentTag);
    	
        if (it != invalidTags.end()) {
            readCoheMiss.inc();
        } else {
            if (cacheTracker -> isCompulsoryMiss(calcTag(addr))) {
                readCompMiss.inc();
            } else {
                readReplMiss.inc();
            }
...
void SMPCache::doWrite(MemRequest *mreq)
{
    ...
    if (l && (l->getState()==DMESI_INVALID || l->getState()==DMESI_SHARED)) {
    	writeCoheMiss.inc();
    } else {
        std::vector<PAddr>::iterator it;	  	
    	PAddr currentTag = calcTag(addr);
...
void SMPCache::realInvalidate(PAddr addr, ushort size, bool writeBack)
{
    while(size) {
        ...
        if (l) {
            ...
            l->invalidate();
            invalidTags.push_back(l->previousTag);
                                       ...
                                       bool canDestroyCB)
{
	...
	std::vector<PAddr>::iterator it;	  	
	PAddr prevTag = l->previousTag;
	it = std::find(invalidTags.begin(), invalidTags.end(), prevTag);
